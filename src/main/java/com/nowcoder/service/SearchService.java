...
import com.nowcoder.model.PageInfo;
...
public class SearchService {
    ...
    public PageInfo<Question> searchQuestion(String keyword, int offset, int count,
                                         String hlPre, String hlPos) throws Exception {
        List<Question> questionList = new ArrayList<>();
        SolrQuery query = new SolrQuery(keyword);
        query.setRows(count);
        query.setStart((offset-1)*10);
        ...
        long sum = response.getResults().getNumFound();
        Map<String, Map<String, List<String>>> highlighting=response.getHighlighting();
        PageInfo<Question> pageInfo = new PageInfo<>();
        pageInfo.setSumInfoCount((int) sum);
        pageInfo.setPageCount();
        pageInfo.setCurPageNum(offset);
        ...
        pageInfo.setCurPageInfoList(questionList);
        return pageInfo;
    }
